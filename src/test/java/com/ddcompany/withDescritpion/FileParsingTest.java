...
//        InputStream stream = cl.getResourceAsStream("sample-xlsx-file.xlsx");//берем наш ранее созданный input stream, передаем в него наш xls файл и кладем все это в локальную переменную
    ...
    void parseCsvFileTest() throws Exception {
        try (InputStream stream = cl.getResourceAsStream("files_Lesson_7/example.csv")) {
    ...
    void zipTest() throws Exception {
        ...
        try (InputStream stream = cl.getResourceAsStream("files_Lesson_7/sample-zip-file.zip");
        ...
        ZipFile zf = new ZipFile(new File(cl.getResource("files_Lesson_7/sample-zip-file.zip").toURI()));
